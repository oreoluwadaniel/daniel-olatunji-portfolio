...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title: `${p.title} | Daniel Olatunji`, description: p.short, url: `${siteUrl}/work/${p.slug}`, images: [{ url: p.cover, width: 1600, height: 900, alt: p.title }] },
  };
}
...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const hasRepo = Boolean(p.github);
  ...
  return <main>
    ...
    <section className="case-hero"><div className="container"><Link className="backlink" href="/work">← All projects</Link><div className="case-hero-grid"><div>
      <div className="eyebrow">{p.category}</div><h1>{p.title}</h1><p className="lead">{p.short}</p><div className="notice">{p.status}</div>
      <div className="actions">{hasRepo ? <a className="btn btn-primary" href={p.github} target="_blank" rel="noreferrer">Open the repository</a> : <span className="btn btn-secondary" aria-disabled="true">Repository not published</span>}<Link className="btn btn-secondary" href="/contact">Talk to me about this</Link></div>
    </div><div><figure className="case-cover"><img src={p.cover} alt={`${p.title}: chart summary of the analysis`} width="1600" height="900" fetchPriority="high" /></figure><p className="caption">Visual summary of the analysis. Full outputs sit in the linked repository when one is published.</p></div></div></div></section>
    ...
    <section className="section-tight"><div className="container contact-box"><div><div className="eyebrow">Review the evidence</div><h2>{hasRepo ? "The code is open. Go and read it." : "The project evidence is documented here."}</h2><p>{hasRepo ? "Queries, transformation logic, validation rules and documentation sit in the linked repository." : "This project is documented from the CV because a public repository is not currently linked."}</p>{hasRepo ? <a className="btn btn-primary" href={p.github} target="_blank" rel="noreferrer">Open the repository</a> : <span className="btn btn-secondary" aria-disabled="true">Repository not published</span>}</div><div><div className="eyebrow">Next project</div><h3 style={{marginTop:8}}>{next.title}</h3><p className="small">{next.short}</p><Link className="btn btn-secondary" href={`/work/${next.slug}`}>Read it →</Link></div></div></section>
  </main>;
}
