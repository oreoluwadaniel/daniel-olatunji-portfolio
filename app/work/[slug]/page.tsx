...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(schema)}} />
    <section className="case-hero"><div className="container"><Link className="backlink" href="/work">← All projects</Link><div className="case-hero-grid"><div><div className="eyebrow">{p.category}</div><h1>{p.title}</h1><p className="lead">{p.short}</p><div className="actions"><a className="btn btn-primary" href={p.github} target="_blank" rel="noreferrer">Open the repository</a><Link className="btn btn-secondary" href="/contact">Talk to me about this</Link></div></div><div><figure className="case-cover"><img src={p.cover} alt={p.title} width="1600" height="900" fetchPriority="high" /></figure></div></div></div></section>
    <section className="container snapshot"><div className="card"><span className="small">Size of the work</span><strong>{p.scale}</strong></div><div className="card"><span className="small">Data used</span><strong>{p.data.slice(0,2).join(" · ")}</strong></div><div className="card"><span className="small">Main tools</span><strong>{p.tools.slice(0,3).join(" · ")}</strong></div><div className="card"><span className="small">Area</span><strong>{p.category.split(" & ")[0]}</strong></div></section>
    <section className="case-section"><div className="container copy"><div className="eyebrow">01 · Context</div><h2>Business context</h2><p>{p.context}</p></div></section>
    <section className="case-section"><div className="container copy"><div className="eyebrow">02 · Decision</div><h2>The decision to support</h2><p>{p.problem}</p><p>{p.objective}</p></div></section>
    <section className="case-section"><div className="container"><div className="eyebrow">03 · Analysis</div><h2>Data and approach</h2><div className="two-col" style={{marginTop:28}}><div><h3>Data</h3><ul className="case-list">{p.data.map(x=><li key={x}>{x}</li>)}</ul></div><div><h3>Questions</h3><ul className="case-list">{p.questions.map(x=><li key={x}>{x}</li>)}</ul></div></div><ol className="case-list numbered" style={{marginTop:30}}>{p.work.map(x=><li key={x}>{x}</li>)}</ol><div className="workflow">{p.workflow.map(x=><div key={x}>{x}</div>)}</div></div></section>
    <section className="case-section"><div className="container"><div className="eyebrow">04 · Findings</div><h2>Key findings</h2><div className="finding-grid">{p.findings.map((x,i)=><div className="card finding" key={x}><span className="fnum">{String(i+1).padStart(2,"0")}</span><p>{x}</p></div>)}</div></div></section>
    {evidence && <section className="case-section evidence-section"><div className="container"><div className="eyebrow">Evidence</div><h2>Project evidence</h2><figure className="evidence-frame"><img src={evidence.src} alt={evidence.alt} loading="lazy" /></figure></div></section>}
    <section className="case-section"><div className="container"><div className="eyebrow">05 · Action</div><h2>Recommended actions</h2><ul className="case-list">{p.recommendations.map(x=><li key={x}>{x}</li>)}</ul></div></section>
    <section className="case-section"><div className="container"><div className="eyebrow">06 · Tools</div><h2>Tools used</h2><div className="tag-list" style={{marginTop:20}}>{p.tools.map(t=><span className="tag" key={t}>{t}</span>)}</div></div></section>
    <section className="section-tight"><div className="container contact-box"><div><div className="eyebrow">Next project</div><h2 style={{marginTop:8}}>{next.title}</h2><p>{next.short}</p><Link className="btn btn-primary" href={`/work/${next.slug}`}>Read it →</Link></div><div><div className="eyebrow">Work with me</div><h3 style={{marginTop:8}}>Have a number nobody can explain?</h3><p className="small">Tell me what the report says and what you suspect it is hiding.</p><Link className="btn btn-secondary" href="/contact">Get in touch</Link></div></div></section>
  </main>;
}
